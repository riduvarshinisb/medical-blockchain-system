...
const UploadBill = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        setResult(res.data);
        toast.success("Bill secured successfully");
    ...
    <div style={S.wrap}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={S.fieldWrap}>
          ...
          <input type="number" value={billAmount} onChange={e => setBillAmount(e.target.value)} required placeholder="0.00" step="0.01" min="0" style={S.input} onFocus={e => e.target.style.borderColor="#1a1a1a"} onBlur={e => e.target.style.borderColor="#e0ddd6"} />
        </div>

        <div style={S.fieldWrap}>
          <label style={S.label}>Bill File</label>
          <label style={S.fileWrap} htmlFor="billFile">
            {file ? (
              <div>
          ...
          <input type="file" id="billFile" accept=".pdf,.jpg,.jpeg,.png" onChange={handleFileChange} style={S.fileInput} />
        </div>
      ...
      {result && (
        <div style={S.successWrap} className="animate-in">
          <div style={S.successLabel}>BILL SECURED SUCCESSFULLY</div>
          <div style={S.successRow}>
            <div style={S.successKey}>BILL ID</div>
            <div style={S.successVal}>#{result.billId}</div>
          </div>
          <div style={S.successRow}>
          ...
          <div style={S.successRow}>
            <div style={S.successKey}>AMOUNT</div>
            <div style={S.successVal}>₹{billAmount}</div>
          </div>
          <div style={S.successRow}>
